...
export const defineAdConfigs = async (baseUrl) => {
  ...
  return defineConfig(async ({ command, mode }) => {
    ...
    const _loggerInfo = filteredLogger.info.bind(filteredLogger);
    filteredLogger.info = (msg, options) => {
      if (msg.includes('new dependencies optimized')) return;
      _loggerInfo(msg, options);
    };
              ...
              if (!isRestart) {
                server.httpServer?.on('listening', () => {
                  setTimeout(() => {
                    ...
                    const bannerRule = '═'.repeat(52);
                    server.config.logger.info('');
                    server.config.logger.info(chalk.whiteBright('  Bringing your ads to life…'));
                    server.config.logger.info('');
                    server.config.logger.info(chalk.greenBright(bannerRule));
                    server.config.logger.info(
