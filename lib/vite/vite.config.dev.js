...
import { devMessages } from '../sayings/dev-messages.js';
import { getNextSaying } from '../sayings/get-next-saying.js';

const HANDLEBARS_RESERVED_KEYS = ['css-variables', 'sprites', 'context', 'variants', 'delayedHover', '__devAdConfigs', '__devCurrentAd', '__devCurrentVariant', 'width', 'height', 'job', 'name'];
...
export const defineAdConfigs = async (baseUrl) => {
  const isUrlAlreadyOpenInBrowser = (url) => {
    ...
    const script = [
      ...
      '          if (URL of t as text) is targetUrl then return "true"',
      '        end repeat',
      '      end repeat',
      '    end tell',
      '  end if',
    ...
    const result = spawnSync('osascript', ['-e', script, url], {
      encoding: 'utf-8',
    });
  ...
  const getDelayedHoverFromJobSettings = () => {
    try {
  ...
  let currentDelayedHover = getDelayedHoverFromJobSettings();
  ...
  for (const [name, config] of Object.entries(adConfigs)) {
    ...
    if (config.context) {
      handlebarsContexts[`/${name}/index.html`] = wrapTemplateVariablesForHandlebars({
        ...config.context,
        delayedHover: currentDelayedHover,
    ...
    if (config.variants) {
      config.variants.forEach((variant) => {
        ...
        handlebarsContexts[`/${name}/${variant.name}.html`] = wrapTemplateVariablesForHandlebars({
          ...mergedContext,
          delayedHover: currentDelayedHover,
          __devAdConfigs: JSON.stringify(devAdConfigs),
          __devCurrentAd: name,
    ...
    return {
      ...
      plugins: [
                ...
                req.on('end', () => {
                  try {
                    ...
                    if (handlebarsContexts[htmlPath]) {
                      ...
                      Object.assign(
                        handlebarsContexts[htmlPath],
                        wrapTemplateVariablesForHandlebars(updatedContext, HANDLEBARS_RESERVED_KEYS),
                      );
                    }
                    
                    console.log(srChalkTheme.success(`Config updated: ${adSize}/${version}`));
                    res.setHeader('Content-Type', 'application/json');
            ...
            server.middlewares.use('/api/ad-config/sync-variable', (req, res) => {
              ...
              req.on('end', () => {
                try {
                  ...
                  // Update handlebars contexts so next page render includes the change
                  // (without this, changes are invisible until server restart)
                  for (const configPath of configFiles) {
                    const fileName = configPath.split('/').pop();
                    const baseName = fileName.replace(/\.config\.(yaml|yml)$/, '');
                    const isBase = baseName === 'ad';
                    const versionKey = isBase ? 'base' : baseName;
                    const htmlPath = isBase ? `/${adSize}/index.html` : `/${adSize}/${versionKey}.html`;
                  ...
                  console.log(srChalkTheme.success(`Variable ${actionLabel} across ${configFiles.length} configs: ${targetLabel}`));
                  res.setHeader('Content-Type', 'application/json');
                ...
                try {
                  ...
                  if (adConfig) {
                    ...
                    if (newVariant) {
                      ...
                      handlebarsContexts[`/${adSize}/${newVersionName}.html`] = wrapTemplateVariablesForHandlebars({
                        ...mergedContext,
                        delayedHover: currentDelayedHover,
                  ...
                  console.log(srChalkTheme.success(`Created new version: ${adSize}/${newVersionName} (from ${source})`));
                  res.setHeader('Content-Type', 'application/json');
                  res.end(JSON.stringify({
                    success: true,
            ...
            server.middlewares.use('/api/delete-version', (req, res) => {
              ...
              req.on('end', () => {
                try {
                  ...
                  console.log(srChalkTheme.success(`Deleted version: ${adSize}/${versionName}`));
                  res.setHeader('Content-Type', 'application/json');
                  res.end(JSON.stringify({
                    success: true,
                    variants: updatedDevAdConfigs.find(a => a.name === adSize)?.variants || [],
        ...
        },
        {
          ...
          configureServer(server) {
            const ws = server.ws;
            if (ws && !ws.__digadSuppressReloadPatchApplied) {
              ...
              ws.__digadSuppressReloadPatchApplied = true;
            }
            ...
            server.middlewares.use('/api/create-size', (req, res) => {
              ...
              req.on('end', () => {
                try {
                  ...
                  // Creating a new size touches many files and can trigger several
                  // watcher-driven full reloads. Temporarily suppress those so the
                  // client-side navigation remains the single visible refresh.
                  suppressFullReloadUntil = Date.now() + 5000;
                  ...
                  // Avoid mutating package.json in dev create-size flow; changing root
                  // config files can trigger additional server refresh/restart cycles.

                  console.log(srChalkTheme.success(`Created new ad size: ${newSizeName} (from ${sourceSize})`));
                  res.setHeader('Content-Type', 'application/json');
                  res.end(JSON.stringify({
                    success: true,
            ...
            server.middlewares.use('/api/delete-size', (req, res) => {
              ...
              req.on('end', () => {
                try {
                  ...
                  console.log(srChalkTheme.success(`Deleted ad size: ${sizeName}`));
                  res.setHeader('Content-Type', 'application/json');
                  res.end(JSON.stringify({
                    success: true,
                    adConfigs: updatedDevAdConfigs,
              ...
              if (req.method === 'POST') {
                ...
                req.on('end', () => {
                  try {
                    ...
                    currentDelayedHover = !!delayedHover;

                    // Keep all in-memory handlebars contexts in sync with global delayedHover
                    for (const key of Object.keys(handlebarsContexts)) {
                      handlebarsContexts[key].delayedHover = currentDelayedHover;
                      delete handlebarsContexts[key].requireMainTimelineCompleteForHover;
                    }

                    console.log(srChalkTheme.success(`Job settings updated: ${jobNumber} / ${jobName}`));
                    res.setHeader('Content-Type', 'application/json');
                    res.end(JSON.stringify({ success: true }));
                  } catch (error) {
                    res.statusCode = 500;
            ...
            server.middlewares.use('/api/save-screenshot', (req, res) => {
              ...
              req.on('end', () => {
                try {
                  ...
                  console.log(srChalkTheme.success(`Screenshot saved: ${adSize}/statics/${filename}`));
              ...
              if (!isRestart) {
                server.httpServer?.on('listening', () => {
                  setTimeout(() => {
                    ...
                    const adsUrl = `${visibleHost}all.html`;
                    if (!didAutoOpenBrowser) {
                      didAutoOpenBrowser = true;
                      if (!isUrlAlreadyOpenInBrowser(adsUrl)) {
                        openUrlInBrowser(adsUrl);
                      }
                    }
                    const devMessage = getNextSaying('dev-messages', devMessages);
                    const divider = '─'.repeat(50);

                    console.log('');
                    console.log(chalk.bold.gray('✔ Ads Ready'));
                    console.log(`  ${chalk.blueBright.bold.underline(`${adsUrl}`)}`);
                    console.log('');
                    console.log(chalk.gray.dim(`  ${divider}`));
                    console.log('');
                    const maxLineWidth = 50;
                    const dashIndex = devMessage.lastIndexOf(' - ');
                    const quote = dashIndex !== -1 ? devMessage.slice(0, dashIndex) : devMessage;
                    const author = dashIndex !== -1 ? devMessage.slice(dashIndex + 1) : '';
                    const words = quote.split(' ');
                    let line = '';
                    for (const word of words) {
                      if (line && (line + ' ' + word).length > maxLineWidth) {
                    ...
                    console.log('');
                    if (author) console.log(chalk.bold.gray(`  ${author}`));
                    console.log('');
                  }, 100);
                });
              }
        ...
        {
          name: 'sprite-sidebar-refresh',
          apply: 'serve',
          configureServer(server) {
            const imageExtRe = /\.(png|jpg|jpeg|gif|svg|webp)$/i;
            const imgPathRe = /\/src\/img\//;
            const spriteSourcePathRe = /\/src\/img\/sprite\//;
            const generatedSpritePathRe = /\/src\/img\/sprite\.png$/i;
            let reloadTimer = null;

            const shouldReloadForFile = (filePath) => {
              const normalized = (filePath || '').replace(/\\/g, '/');
              if (!imgPathRe.test(normalized)) return false;
              if (normalized.endsWith('/.gitkeep')) return false;

              // Ignore sprite source files; sprite.png change is the signal that
              // spritesmith has finished regenerating assets.
              if (spriteSourcePathRe.test(normalized)) return false;
              if (generatedSpritePathRe.test(normalized)) return true;

              return imageExtRe.test(normalized);
            };

            const queueReload = (filePath) => {
              if (!shouldReloadForFile(filePath)) return;
              if (reloadTimer) {
                clearTimeout(reloadTimer);
              }

              // Debounce bursts (add + spritesheet regenerate) into one reload.
