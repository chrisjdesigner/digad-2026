...
// Load chevron SVG at module initialization (same as render.js)
let chevronDownIconSvg;
try {
  const renderDirPath = dirname(new URL(import.meta.url).pathname);
  chevronDownIconSvg = readFileSync(resolve(renderDirPath, '../dev-mode/icons/chevron-down.svg'), 'utf-8')
    .replace(/\s+/g, ' ')
    .trim()
        ...
        if (url === '/all.html') {
          ...
          const allCardsHtml = Object.entries(adConfigs).map(([adSize, config]) => {
            ...
            return '<section class="size-group" data-size-group="' + adSize + '">' +
              '<h2 class="size-group-title">' + chevronDownIconSvg + '<span class="size-group-title-text">' + adSize + '</span></h2>' +
