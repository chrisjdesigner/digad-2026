...
// printSection('Package Build');
// console.log(chalk.gray('Package build pipeline'));
...
console.log(`${chalk.blueBright('  ✓')} ${chalk.blueBright('TypeScript check complete')}`);
...
for (const size of adSizeDirs) {
  ...
  console.log(
    `${chalk.blueBright('✓')}  ${chalk.blueBright(`${size} build complete`)} ` +
...
if (zipReports.length > 0) {
  ...
  console.log(`\n${chalk.gray(tableRule)}`);
  console.log(chalk.gray.bold('  Platform Size Limits'));
  console.log(chalk.gray(tableRule));
  ...
  const rule = '═'.repeat(78);
  console.log(chalk.gray(tableRule));
  console.log(chalk.whiteBright.bold('  Final ZIP Sizes (Actual File Size)'));
  console.log(chalk.gray(tableRule));
...
console.log(`\n${chalk.blueBright.bold('All set. Your ads are packaged and ready to go.')}\n`);
