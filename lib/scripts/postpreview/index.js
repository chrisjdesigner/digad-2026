...
const PREVIEW_VERBOSE = process.env.DIGAD_PREVIEW_VERBOSE === '1';
...
try {
  ...
  info(chalk.blueBright.bold('Preview URLs:'));
  ...
  info('Could not read global digads config file');
  info('Could not generate preview URLs');
}
...
try {
  ...
  info(chalk.blueBright('Generated preview/all.html with all ad sizes and versions'));
  ...
  for (const [adSize, config] of Object.entries(adConfigs)) {
    ...
    info(chalk.blueBright(`Generated preview/${adSize}/all.html`));
  }
} catch (error) {
  ...
  try {
    ...
    for (const [adSize, config] of Object.entries(adConfigs)) {
      ...
      info(chalk.blueBright(`Injected configs into preview/${adSize}/ HTML files`));
    }
  } catch (error) {
    console.error(chalk.redBright('Error injecting configs into individual ad pages:'), error.message);
  }
  
  const localPreviewUrl = pathToFileURL(resolve(baseDirectory, 'preview', 'all.html')).href;
  const localPreviewHttpUrl = 'http://localhost:5173/preview/all.html';
  const firstAdConfig = Object.values(adConfigs)[0];
  const deployedPreviewUrl = visibleHost && firstAdConfig?.context?.client && firstAdConfig?.context?.job
    ? `${visibleHost}${firstAdConfig.context.client}/${firstAdConfig.context.job}/preview/all.html`
    : null;
  const previewUrl = deployedPreviewUrl || localPreviewHttpUrl;
  const divider = '─'.repeat(78);

  console.log(chalk.gray(divider));
  console.log('');
  console.log(chalk.white.dim('✔ Preview Links Ready'));
  console.log('');
  console.log(chalk.blueBright.bold.underline(previewUrl));
  console.log('');
  console.log(chalk.gray(divider));
  console.log('');
  console.log(chalk.gray('Push Files to Deploy on Staging'));
  console.log('');
