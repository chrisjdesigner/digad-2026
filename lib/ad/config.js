...
export function loadAdConfigs(rootDirectory) {
  ...
  for (const adSize of adSizeDirectories) {
    ...
    const versionsFolder = resolve(adFolder, 'versions');

    // Load base config (prefer versions/ folder, then root, prefer YAML over JSON)
    let baseConfigPath = null;
    if (existsSync(versionsFolder)) {
      baseConfigPath = findConfigFile(resolve(versionsFolder, 'ad.config'));
    }
    if (!baseConfigPath) {
      baseConfigPath = findConfigFile(resolve(adFolder, 'ad.config'));
    }
    ...
    // Look in both the ad folder root and the versions/ subfolder
    const variantConfigFiles = [];
    
    // Check root folder
    variantConfigFiles.push(
      ...readdirSync(adFolder)
        .filter((file) => {
          const isConfigFile =
            file.endsWith('.config.yaml') ||
            file.endsWith('.config.yml') ||
            file.endsWith('.config.json');
          const isNotBaseConfig = !file.startsWith('ad.config.');
    ...
    // Check versions/ subfolder if it exists
    if (existsSync(versionsFolder)) {
      variantConfigFiles.push(
        ...readdirSync(versionsFolder)
          ...
          .map((file) => ({
            name: file.replace(/\.config\.(yaml|yml|json)$/, ''),
            path: resolve(versionsFolder, file),
          }))
      );
    }
